...
def initialize_joystick():
    try:
        ...
        print("ジョイスティックの初期化中にエラーが発生しました。")
        return None
...
if __name__ == '__main__':
    Init_speed = 5  # 1-15  スピードの初期値（1から15の範囲）
    turn = 0
    ...
    turtle.setup(800, 800)  # Turtleウィンドウのサイズを設定
    turtle.speed(0)  # 最速の描画スピードに設定
    turtle.pensize(3)  # 描画のペンサイズを設定
    ...
    while start:
        ...
        # turtle.clear()  # Turtleの描画をクリア
        # turtle.goto(0, 0)  # Turtleの位置をリセット
        turtle.setheading(angle)  # Turtleの向きを設定

        # Turtleに線を描画
        turtle.pendown()
        turtle.pencolor(RED_COLOR[0] / 255, RED_COLOR[1] / 255, RED_COLOR[2] / 255)
        turtle.forward(speed)

        turtle.update()  # Turtleウィンドウの更新

    turtle.done()  # Turtleを終了